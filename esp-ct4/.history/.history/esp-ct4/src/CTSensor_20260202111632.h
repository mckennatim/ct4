...
public:
    ...
    void setup() override {
        // Initialization handled in Factory or not needed
        Serial.printf("Setup CTSensor Ch:%d\n", _config.pin);
    }
    ...
    // Static helper to find the zero point (DC Bias)
    static void calibrateZero(Adafruit_ADS1115* ads) {
        Serial.println("... Calibrating ADC Zero (DC Bias) ...");
        long total = 0;
        int count = 50;
        // Read channel 0 (assuming similar bias for all)
        for(int i=0; i<count; i++) {
            total += ads->readADC_SingleEnded(0);
            delay(2);
        }
        _zeroOffsetADC = total / (float)count;
        Serial.printf("Zero Offset: %.1f\n", _zeroOffsetADC);
    }
};

// Define statics (Allocating storage)
...
public:
    static void load(std::vector<Sensor*>& sensors, MqttManager* mqtt) {
        ...
        for (const auto& cfg : ct_sensors) {
            if (String(cfg.name) == "empty") continue;
