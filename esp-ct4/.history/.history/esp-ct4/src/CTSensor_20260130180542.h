...
private:
    ...
    float readCurrent() {
        ...
        // REVERTED TO HIGH ACCURACY: 2000 samples (~15 seconds @ 128SPS)
        // STRATEGY: "Yield" to MQTT every ~0.4s to prevent disconnection/lag.
        ...
        for (int i = 0; i < samples; i++) {
            ...
            // CRITICAL: Keep network alive during this long blocking operation!
            // Check network every 50 samples (~390ms)
            if ((i % 50) == 0) {
                 if (_mqtt) _mqtt->loop();
            }
        }
